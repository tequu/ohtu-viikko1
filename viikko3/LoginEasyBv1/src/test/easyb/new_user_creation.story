...
scenario "creation fails with number in username and valid password", {
    given 'command new user is selected', {
       userDao = new InMemoryUserDao()
       auth = new AuthenticationService(userDao)
       io = new StubIO("new", "1eero", "sala1nen")
       app = new App(io, auth)
    }
    when 'username contain number and valid password are entered', {
      app.run()
    }
    then 'new user is not be registered to system', {
       io.getPrints().shouldHave("new user not registered")
